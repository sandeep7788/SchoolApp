...
import com.education.vidhyalayaaa.API.Changepasswordapi
...
class ChangePassword : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Picasso.with(this@ChangePassword).load(Data.imageBaseUrl+getdata(Data.student_image))
            .error(R.drawable.ic_account_circle_black_24dp)
            .into(imageview)
        t_name.setText(getdata(Data.Student))
    ...
    fun change_password()
    {
        ...
        val loginResponseCall: Call<Changepasswordapi> = preferences.getchange_password()!!.getchancepasswordservices(getdata(Data.Userid)!!.trim(),e_oldpassword.text.toString(),e_newpassword.text.toString())
        loginResponseCall.enqueue(object : Callback<Changepasswordapi>
        {
            ...
            override fun onResponse(
                ...
                if (response.isSuccessful) {
                   if(response.body()!!.status.toString().equals("Invalid Old Password")) {
                        showDialog("Your request not submitted",SweetAlertDialog.WARNING_TYPE)
                    } else if(response.body()!!.status.toString().equals("Password Change Successfully")) {
                        showDialog(response.body()!!.status.toString(),SweetAlertDialog.SUCCESS_TYPE)
                    } else {
                        showDialog(response.body()!!.status.toString(),SweetAlertDialog.WARNING_TYPE)
                    }

                } else {
                    showDialog("Your request not submitted",SweetAlertDialog.ERROR_TYPE)
                }
                pb.dismiss()
            }
        })
    ...
    fun showDialog(title:String,type:Int) {
        ...
        if (pausingDialog == null) {
            pausingDialog = SweetAlertDialog(this, type)
