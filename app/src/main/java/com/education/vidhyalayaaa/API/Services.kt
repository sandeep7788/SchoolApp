...
//    val baseurl="http://vidhyalaya.co.in/sspanel/API/holiday/"
    ...
    var mMarkSheet: MarkSheet? = null
    fun getMarkSheet(): MarkSheet? {
        if (dashbord_menu == null) {
            val retrofit = Retrofit.Builder()
                .client(okHttpClient)
                .baseUrl(baseurl)
                .addConverterFactory(GsonConverterFactory.create())
                .build()
            mMarkSheet = retrofit.create(MarkSheet::class.java)
        }
        return mMarkSheet
    }
    ...
    //    http://vidhyalaya.co.in/sspanel/API/holiday/marksheet?school_id=28&stdid=43969&school_session=2021-2022
    //    http://vidhyalaya.co.in/sspanel/API/holiday/marksheet?school_id=28&stdid=43969&school_session=2021-2022&exam_id=23
    interface MarkSheet {
        @GET("marksheet?")
        fun getMarkSheet(
            @Query("school_id") one: String,
            @Query("stdid") two: String,
            @Query("school_session") key: String,
            @Query("exam_id") exam_id: String
        ): Call<List<MarkSheetModel>>

//        http://vidhyalaya.co.in/sspanel/API/holiday/getExamName?school_id=28
        @GET("getExamName?")
        fun getExamType(
            @Query("school_id") school_id: String
        ): Call<List<ExamTypeModel>>
    }
